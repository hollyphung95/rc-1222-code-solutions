...
pokedex.forEach((value, index, pokedex) => {
  ...
  imgElement.setAttribute('src', value.imageUrl);
...
function renderPokemon(pokemon) {
  ...
  $imgElement.setAttribute('src', pokemon.getAttribute('src'));
  $card.appendChild($imgElement);
  var $text = document.createElement('div');
  $text.classList.add('pokemon-card-text');
  $card.appendChild($text);
  var $nameHeader = document.createElement('h2');
  $nameHeader.innerText = pokemon.getAttribute('name');
  $text.appendChild($nameHeader);
  var $numHeader = document.createElement('h3');
  $numHeader.innerText = `#${pokemon.getAttribute('number')}`;
  $text.appendChild($numHeader);

  var $paragraph = document.createElement('p');
  $paragraph.innerText = `#${pokemon.getAttribute('description')}`;
